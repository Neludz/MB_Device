...
static bool invalid_frame(MBStruct_t *mbb)
{
    uint8_t *buf_ptr;
    uint32_t PDU_len;
#if (MB_TCP_PERMISSION == 1)
    if (mbb->mb_frame_type == MB_TYPE_TCP)
    {
        buf_ptr = &mbb->p_mb_buff[6];
        PDU_len = (mbb->p_mb_buff[4] << 8) | (mbb->p_mb_buff[5] & 0xFF);
        if ((PDU_len >= MB_FRAME_MAX) || ((mbb->mb_index - 6) != PDU_len))
        {
            return true;
        }
    }
    ...
    if (PDU_len < MB_FRAME_MIN)
    {
        return true;
    }
    if ((mbb->slave_address != buf_ptr[0]) && (buf_ptr[0] != MB_ADDRESS_BROADCAST) && (mbb->slave_address != 0))
    {
        return true;
    }
#if (MB_CALLBACK_REG == 1)
...
#endif
#if (MB_TCP_PERMISSION == 1)
    if (mbb->mb_frame_type == MB_TYPE_RTU)
    {
        if (mb_CRC16((uint8_t *)buf_ptr, PDU_len))
        {
            return true;
        }
    }
#else
    if (mb_CRC16((uint8_t *)mbb->p_mb_buff, mbb->mb_index))
    {
        return true;
    }
#endif
...
static bool frame_parse(MBStruct_t *mbb)
{
    ...
    if (buf_ptr[0] == MB_ADDRESS_BROADCAST)
    {
    ...
    case MB_FUNC_READ_HOLDING_REGISTER:
        ...
        if (8 == PDU_len)
        {
            ...
            RegIndx = (buf_ptr[2] << 8) | (buf_ptr[3] & 0xFF);
            RegNmb = (buf_ptr[4] << 8) | (buf_ptr[5] & 0xFF);
    ...
    case MB_FUNC_WRITE_MULTIPLE_REGISTERS:
        ...
        if (PDU_len > 10)
        {
            ...
            while (RegIndx < RegLast)
            {
                *(mbb->p_write + RegIndx) = buf_ptr[i++] << 8; // High, then Low byte
                *(mbb->p_write + RegIndx) |= buf_ptr[i++];     // ... are packed in a WORD
                ++RegIndx;
            }
    ...
    {
        // Any exception?
        buf_ptr[1] |= MB_FUNC_ERROR; // Add 0x80 to function code
        buf_ptr[2] = Exception;      // Exception code
